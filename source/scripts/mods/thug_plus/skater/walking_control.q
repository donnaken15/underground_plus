			...
			if NOT GetGlobalFlag flag = FLAG_G_EXPERT_MODE_NO_WALKING
				if NOT GetGlobalFlag flag = FLAG_EXPERT_MODE_NO_WALKING
					if (UNDERGROUNDPLUS_CONTROLS = CONTROL_SCHEME_PS2)
						...
						SetQueueTricks Jumptricks WalkToSkateTransition_GroundTricks SkateToWalkTricks_XBOX
					endif
				else
		...
		if NOT IsBoardMissing
			...
			if NOT GotParam WalkingGroundTricksInactive
				if (UNDERGROUNDPLUS_CONTROLS = CONTROL_SCHEME_PS2)
					...
					SetQueueTricks WalkGroundTricks_XBOX
				endif
			else
	...
	script SetWalkingAirTricks
		if NOT IsBoardMissing
			if (UNDERGROUNDPLUS_CONTROLS = CONTROL_SCHEME_PS2)
				...
				SetQueueTricks special = SpecialTricks AirTricks WalkAirTricks_XBOX
			endif
	...
	script SkateToAirState
		...
		ClearExceptionGroup WalkingStateExceptions
		SetException Ex = Land Scr = LandState Group = WalkingStateExceptions
		SetException Ex = Hang Scr = AirToHangState Params = {ksk_speed = <Speed>} Group = WalkingStateExceptions
		SetException Ex = Ladder Scr = LadderMoveUpState Group = WalkingStateExceptions
		SetException Ex = Wallplant Scr = WallplantState Group = WalkingStateExceptions
		SetException Ex = Wallplant Scr = WallplantState Group = WalkingStateExceptions
	...
	script StandState
		...
		while
			...
			UndergroundPlus_IfPressedReturnBoard
			if (<ksk_is_held> = 1)
				SwitchToSkatingPhysics
				skater::ReturnBoardToSkater
				Goto Ksk_StartSkating
			endif
			if (<ksk_is_held> = 2)
				SwitchToSkatingPhysics
				skater::ReturnBoardToSkater
	...
	script RunJumpState
		...
		SetException Ex = Ladder Scr = LadderMoveUpState Group = WalkingStateExceptions
		SetException Ex = Wallplant Scr = WallplantState Group = WalkingStateExceptions
		SetExtraTricks Walking_Air_ExtraTricks
	...
	script StandLandState
		...
		if Held Black
			PlayWalkAnim Anim = BigJumpLandToWalk BlendPeriod = 0.05
		else
	...
	script RunLandState
		...
		if Held Black
			RunSpeedChosenAnim Anims = [BigJumpLandToWalk BigJumpLandToWalk BigJumpLandToWalk BigJumpLandToWalk] Speeds = [1.0 1.20000005 1.5 1.5] BlendPeriod = 0.1 NoSpeedUpdate
	...
	script AirToHangState
		...
		switch <HangInitAnimType>
			case SWING
				if GotParam ksk_speed
					if (<ksk_speed> > HANG_SPINOUT_SPEED)
						UndergroundPlus_IfPressedRailSwing
						if (<ksk_is_held> = 1)
							Goto HangSpinOutState Params = {exit_speed = <ksk_speed>}
						endif
					endif
				endif
				if IsBoardMissing
					if Held White
						Goto AirToGibbonState
					endif
				endif
